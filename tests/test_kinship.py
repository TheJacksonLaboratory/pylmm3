...
def test_blocked_matches_full_matrix(genotypes, block_size):
    ...
    n = genotypes.shape[0]
    K_blocked = calculateKinshipBlocks(_column_blocks(genotypes, block_size), n_samples=n)
...
def test_blocked_matches_full_with_invariant_and_missing(block_size):
    ...
    K_blocked = calculateKinshipBlocks(_column_blocks(W, block_size), n_samples=W.shape[0])
...
def test_blocked_does_not_mutate_input_blocks():
    ...
    calculateKinshipBlocks(blocks, n_samples=W.shape[0])
...
def test_blocked_all_invariant_raises():
    ...
    with pytest.raises(NoVariantSNPsError):
        calculateKinshipBlocks(blocks, n_samples=5)
...
def test_blocked_center_matches_full(genotypes, block_size):
    ...
    n = genotypes.shape[0]
    K_blocked = calculateKinshipBlocks(
